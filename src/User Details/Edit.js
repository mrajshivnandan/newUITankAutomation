...
const Edit = () => {
  ...
  const loadSupplyList= async()=>{
    ...
    if(slist){
      ...
      console.log(result);
      Object.entries(result).forEach((e) => {setFieldValue(e[0],e[1])});
  ...
  const checkRoom = async (values) => {
    ...
    const result = slist[id-1];
    if(values.room === result.room && values.wing === result.wing){
      return errors
    }
    if (slist.filter(function (e) { return e.room === values.room; }).length > 0) {
    sameroom = true;
    }
    if (slist.filter(function (e) { return e.wing === values.wing; }).length > 0) {
    samewing = true;
    }
